...
The model above was based on on a hypothesized structure for the factors that underly birthweight. It was not based on a data-driven model-building process.
...
The distributions of both of the plots above appears to be normal. The median of the log(β0∗β1) value is `r round(median(pull(bootstrap_results, log_b0b1)), digits=2)`, and the mean is `r round(mean(pull(bootstrap_results, log_b0b1)), digits=2)`. The median of the r^2 value is `r round(median(pull(bootstrap_results, r2)), digits=2)`, and the mean is `r round(mean(pull(bootstrap_results, r2)), digits=2)`. 

The 95% confidence interval of log(β0∗β1) is (`r round(quantile(pull(bootstrap_results, log_b0b1), probs = c(0.025,0.975)), digits = 2)`). The 95% confidence interval of r^2 is (`r round(quantile(pull(bootstrap_results, r2), probs = c(0.025,0.975)), digits = 2)`).
